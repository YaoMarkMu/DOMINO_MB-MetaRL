...
from domino.utils.utils import remove_scope_from_name
from domino.dynamics.core.utils import *
...
class MultiHeadedEnsembleContextPredictor(Layer):
    ...
    def ema_update(self,tau=0.9):
        for i in range(len(self.context_hidden_sizes)):
            online = self._params["online_contra_{}_weight".format(i)]
            target = self._params["target_contra_{}_weight".format(i)]
            tf.compat.v1.assign(target,(tau*target + (1-tau)*online))
            # print("######")
            # print("ema update")
