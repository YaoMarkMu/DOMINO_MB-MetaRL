from domino.dynamics.core.layers import MCLMultiHeadedCaDMEnsembleMLP
from domino.dynamics.core.layers import MultiHeadedEnsembleContextPredictor
...
class MCLMultiHeadedCaDMDynamicsModel(Serializable):
    ...
    def __init__(
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            with tf.compat.v1.variable_scope("context_model"):
                ...
                self.bs_contrastive_loss = (
                    self.cp.contrastive_loss
                )  # [head_size, ensemble_size, None, context_dim]
            ...
            self.contrastive_loss = tf.reduce_mean(self.bs_contrastive_loss)
            mse_loss = 0.0
            ...
            self.loss+=tf.clip_by_value(self.contrastive_loss,-10000,10000)*self.cons_lr
            self.ie_loss+=tf.clip_by_value(self.contrastive_loss,-10000,10000)*self.cons_lr
        ...
        for epoch in range(epochs):
            ...
            for batch_num in range(
                ...
                if itr < self.ie_itrs and self.use_ie:
                    ...
                    #self.cp.ema_update()
                    # print("contrastive_loss",contrastive_l)
    ...
    def _preprocess_inputs(
        ...
        # print(_future_bool.shape)
        # #print(_cp_act[_future_bool > 0, :].shape)
        # print(_sim_param.shape)
        # print((_future_bool > 0).shape)
        # print(_sim_param[_future_bool > 0, :].shape)
        # input()
