...
from domino import spaces
from domino.envs.base import Env
from domino.mujoco_py import MjModel, MjViewer
...
class MujocoEnv(Env):
    ...
    def __init__(self, file_path=None, action_noise=0, random_init_state=True):
        # compile template
        ...
        self.np_random = seeding.np_random(None)[0]  # default random seed.
        self.reset_()
        ...
        if init_state is None:
            if self.random_init_state:
                self.model.data.qpos = (
                    self.init_qpos
                    + self.np_random.normal(size=self.init_qpos.shape) * 0.01
                )
                self.model.data.qvel = (
                    self.init_qvel
                    + self.np_random.normal(size=self.init_qvel.shape) * 0.1
                )
            else:
    ...
    def reset_(self, init_state=None):
    ...
    def inject_action_noise(self, action):
        ...
        noise = self.action_noise * self.np_random.normal(size=action.shape)
    ...
    def release(self):
        ...
        from domino.mujoco_py.mjlib import mjlib
